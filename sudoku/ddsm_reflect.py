...
#         )
    ...
    # def step(x, step_size):
    #     g = torch.sqrt(s * x * (1 - x))  # Diffusion coefficient respects boundaries
    #     drift = 0.5 * s * (a * (1 - x) - b * x)  # Drift term
    #     dx = x + drift * step_size  # Euler step for the drift
    #     x_next = dx + torch.sqrt(step_size) * g * torch.randn_like(x)  # Add diffusion term

    #     # Reflect boundaries to ensure the process stays within [0, 1]
    #     x_next = torch.clamp(x_next, eps, 1 - eps)
    #     # Applying reflection at the boundaries
    #     x_next = torch.where(x_next <= 0, -x_next, x_next)
    #     x_next = torch.where(x_next >= 1, 2 - x_next, x_next)
...
################ modfy second time ###########
def reflected_sde(X, t, f, g, dt):
    dXt = f(X, t) * dt + g(t).view(-1, 1, 1, 1) * torch.randn_like(X, device=X.device) * torch.sqrt(torch.tensor(dt, device=X.device))
    #dXt = f(X, t) * dt + g(t).unsqueeze(-1).unsqueeze(-1) * torch.randn_like(X) * torch.sqrt(torch.tensor(dt, device=X.device))
    #dXt = f(X, t) * dt + g(t) * torch.randn_like(X) * torch.sqrt(torch.tensor(dt, device=X.device))
    X_new = X + dXt
    # check boundary
    X_new = reflect(X_new)
    # X_new = torch.max(X_new, torch.tensor(0.0, device=X.device))  # 确保数据不小于0
    # X_new = torch.min(X_new, torch.tensor(1.0, device=X.device))  # 确保数据不大于1
    return X_new

def cdsm_loss(score_model, X0, Xt, t, lambda_t):
    # Calculate constrained denoising score matching loss
    loss = 0
    # for i in range(len(X0)):
    #     score = score_model(Xt[i], t)
    #     target = (Xt[i] - X0[i]) / lambda_t  # Calculate true score
    #     loss += np.sum((score - target)**2)
    # return loss / len(X0)
    batch_size = X0.shape[0]
    for i in range(batch_size):
        score = score_model(Xt[i].unsqueeze(0), t[i].unsqueeze(0))
        target = (Xt[i] - X0[i]) / lambda_t[i]  # Calculate true score
        loss += torch.sum((score - target.unsqueeze(0))**2)
    return loss / batch_size


def sample_reflected_sde(score_model, T, dt, X0):
    X = X0
    t_values = np.arange(0, T, dt)
    for t in reversed(t_values):
        dXt = reflected_sde(X, t, lambda X, t: -score_model(X, t), lambda t: np.sqrt(t), dt)
        X = X + dXt
    return X
...
def diffusion_factory(
    ...
    print(v_samples.device)
    print(v_samples_grad.device)
...
def diffusion_factory_add(
    ...
    sample_inds = torch.randint(0, noise_factory_size, size=x.size()[:-1]).to(device)
######################### S
    print(sample_inds.device)
    sample_inds = sample_inds.to(noise_factory_zero.device)
    time_ind = time_ind.to(noise_factory_zero.device)
######################### E
    v_samples = noise_factory_zero[sample_inds, time_ind, :].float()
    v_samples_grad = (
        noise_factory_zero_loggrad[sample_inds, time_ind, :].float()
    )
######################## S
    print(v_samples.device)
    print(v_samples_grad.device)
